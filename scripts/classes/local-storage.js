...
export class LocalStorage {
  static getLocalStorageData = () => {
    ...
    const transformedBasket = basket.map(item => ({id: Number(item.id), quantity: Number(item.quantity)}))
    const totalPrice = Number(localStorage.getItem(LOCAL_STORAGE_KEYS.TOTAL_PRICE)) ?? 0
    const totalQuantity = Number(localStorage.getItem(LOCAL_STORAGE_KEYS.TOTAL_QUANTITY)) ?? 0
  ...
  static updateLocalStorage = (basket, totalPrice, totalQuantity) => {
    ...
    localStorage.setItem(LOCAL_STORAGE_KEYS.TOTAL_QUANTITY, String(totalQuantity))
  }
}