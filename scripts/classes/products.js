...
import {
  basketModalEmpty,
  basketModalTemplate,
} from '../constants/basket-modal-template.js'
...
export class Products {
  ...
  classes = {
    isOpen: 'product-modal--open',
    noScroll: 'no-scroll',
    active: 'products__categories-button--active',
  }
  ...
  constructor(products, categories) {
    this.topProductsContainer = document.querySelector(
      this.selectors.topProducts,
    )
    this.productsContainer = document.querySelector(
      this.selectors.productsContainer,
    )
    this.basketContainer = document.querySelector(
      this.selectors.basketContainer,
    )
    this.basketPriceContainer = document.querySelector(
      this.selectors.basketPrice,
    )
    this.basketTotalQuantityContainer = document.querySelector(
      this.selectors.basketTotalQuantity,
    )
    ...
    this.productModalWrapper = document.querySelector(
      this.selectors.productModalWrapper,
    )
    this.productModalImage = document.querySelector(
      this.selectors.productModalImage,
    )
    this.productModalTitle = document.querySelector(
      this.selectors.productModalTitle,
    )
    this.productModalDescription = document.querySelector(
      this.selectors.productModalDescription,
    )
    this.productModalPrice = document.querySelector(
      this.selectors.productModalPrice,
    )
    this.productModalButtonsContainer = document.querySelector(
      this.selectors.productModalButtonsContainer,
    )
    this.productModalClose = document.querySelector(
      this.selectors.productModalClose,
    )

    this.categoriesContainer = document.querySelector(
      this.selectors.categoriesContainer,
    )

    this.root = document.querySelector(this.selectors.root)

    const [basket, totalPrice, totalQuantity] =
  ...
  setTopProducts = () => {
    ...
    this.topProductsContainer.innerHTML = popularProducts
      .map(({ id, title, price, image }) => {
        return cardTemplate(
          ...
          price,
          image,
          this.basket.find((item) => item.id === id)?.quantity ?? 0,
        )
      })
      .join('')
  }
  ...
  setProductsContainer = () => {
    if (this.currentCategory === INITIAL_CATEGORY) {
      return (this.productsContainer.innerHTML = this.products
        .map(({ id, title, price, image }) => {
          return cardTemplate(
            id,
            title,
    ...
    const productToShow = this.products.filter(
      ({ type }) => type === this.currentCategory,
    )
    return (this.productsContainer.innerHTML = productToShow
      .map(({ id, title, price, image }) => {
        return cardTemplate(
          id,
          title,
  ...
  setCategories = () => {
    ...
    this.categoriesContainer.innerHTML = allCategories
      .map((category) => {
  ...
  setBasket = () => {
    ...
    this.basketContainer.innerHTML = this.basket
      .map(({ id, quantity }) => {
        const currentProduct = this.products.find((item) => item.id === id)
        return [...Array(quantity)]
          .map((_) => {
            return basketModalTemplate(
              id,
  ...
  setCurrentCategory = (event) => {
    ...
    if (button) {
      const currentCategory =
        button.getAttribute(BUTTON_TYPES.CATEGORY) ?? INITIAL_CATEGORY
      if (currentCategory !== this.currentCategory) {
        const buttons = this.categoriesContainer.querySelectorAll('button')
        buttons.forEach((item) => {
          item.classList.toggle(
            this.classes.active,
            currentCategory === item.getAttribute(BUTTON_TYPES.CATEGORY),
          )
        })
  ...
  onChangeProductQuantity = (buttonType, productId) => {
    ...
    const currentProductPrice = this.products.find(
    ...
    this.totalQuantity = this.basket.reduce(
  ...
  onProductModalOpen = (productId) => {
    ...
    const quantity =
      this.basket.find(({ id }) => id === productId)?.quantity ?? 0
    if (this.productModalWrapper.classList.contains(this.classes.isOpen)) {
      return (this.productModalButtonsContainer.innerHTML =
        productModalButtonsTemplate(productId, quantity))
    }
    ...
    this.productModalButtonsContainer.innerHTML = productModalButtonsTemplate(
      productId,
      quantity,
    )
  ...
  bindEvents = () => {
    ...
    this.productModalButtonsContainer.addEventListener(
      'click',
